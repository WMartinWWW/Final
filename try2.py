...
def load_data():
    ...
    df_sales['Date'] = pd.to_datetime(df_sales['YEAR'].astype(str) + '-' + df_sales['MONTH'].astype(str), errors='coerce')
...
def plot_dynamic_time_series(df_sales):
    ...
    if pd.isnull(min_date) or pd.isnull(max_date):
        st.error("Date range is invalid due to improper date data.")
        return None
...
# Function to plot supplier performance
